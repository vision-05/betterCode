...
better::Text better::backspace(better::Text text) { //couple of bugs with backspace, figure out once got all chars rendering
    ...
    if(text.cursor.column > 0) {
        ...
        if(text.textEdit[text.cursor.row].size() > 0) {
            ...
            return {text.textEdit.erase(text.cursor.row).set(text.cursor.row - 1, newLine), {text.cursor.row - 1, colNo}, {{false,false,false,false},text.data.isShift,text.data.isCaps,text.data.isScroll,text.data.clearHistory,-1,text.data.menu,text.data.filename}, text.cursor.row >= text.textEdit.size() - 1 ? text.topLineNumber - 1 : text.topLineNumber, topColumnNo, text.highlightStart, text.highlightEnd};
        }
    ...
    return {text.textEdit.set(text.cursor.row,newLine), {text.cursor.row, text.cursor.column - 1}, {{false,false,false,false},text.data.isShift,text.data.isCaps,text.data.isScroll,text.data.clearHistory,-1,text.data.menu,text.data.filename}, text.topLineNumber, text.topColumnNumber, text.highlightStart, text.highlightEnd};
}
