...
void better::createLetter(SDL_Surface* surface, char letter, int column, int row) {
    Uint32 pixelGrid[12][8] {
        {255,255,255,255,255,255,255,255}, //store chars as 2d array of 12x8 pixels 
    ...
    for(int i{}; i < 12; ++i) {
        for(int j{}; j < 8; ++j) {
            better::setPixel(surface, i, j, pixelGrid[i][j], column, row, 8, 12); //basically set each pixel for a character
        }
    }
}
