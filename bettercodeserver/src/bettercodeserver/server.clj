...
(defn parse-request
  ...
  (case (message 0)
    ...
    "insert-text" (buffer/insert-text agent-name (message 1) (message 2) (message 3))
    "remove-text" (buffer/remove-text agent-name (message 1) (message 2) (message 3))
                   ...
                   (when-not (= ::none msg)
                     (d/let-flow [msg-two (d/future (f msg files-agent))
                                  result (cond
                                           ...
                                           :else @(s/put! s true))]
                                 (when result
                                   (let-when [file-name (first @files-agent)]
